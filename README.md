...
- **Issue → branch → PR**: every issue becomes exactly one GitHub branch, worked in an isolated workspace, linked to exactly one pull request; merging the PR ships the functionality.
- **Watch CI after every push**: work isn't done (and the next task doesn't start) until CI is green.
- **Planning** creates phase issues + task sub-issues and always ends with the execution-lanes diagram (which tasks run in parallel).
...
Both analysis workflows need the `CLAUDE_CODE_OAUTH_TOKEN` secret (`claude setup-token`); report language is set in each prompt (currently pt-BR) — edit freely.
...
- **`hooks/sync.sh`** — PostToolUse reminder to re-run the `sync` skill after planning docs change.
...
| `plan` | generalized | Interview-driven planning into GitHub Issues: phases = issues, tasks = sub-issues, issue→branch→PR delivery, and every plan ends with the execution-lanes parallelism diagram. |
...
Conventions (one language, INDEX-as-projection, surprise-only learnings, ADR immutability, executable runbooks) in `docs/README.md`, plus fill-in templates for ADRs, learnings, runbooks, and INDEX files.
