---
name: plan
description: Analyze, plan, and create GitHub issues for feature work. Use WHENEVER the user invokes /plan, asks to "create a plan", "plan feature X", "do planning for Y", "new plan", or similar. Runs a one-question-at-a-time discovery interview until clarity ≥95%, explores the codebase, then creates issues — one per phase when phases are needed, with each phase's tasks as sub-issues — and always ends with the execution order showing which tasks run in parallel. The tracker is the planning source of truth.
model: opus
---

# Skill: `plan`

Analyzes work, interviews the operator until clarity, and creates **tracker issues** (GitHub Issues by default; adaptable to Linear or similar). Read your project's `AGENTS.md` and structure doc before planning.
...
- User asks to archive or reorder planned work — update tracker issues instead of moving files.

## Delivery model (inviolable)

Every issue created here follows the project's delivery workflow:

- **1 issue = 1 GitHub branch**, worked in an **isolated workspace** (worktree or fresh session).
- **1 branch = 1 pull request.** Merging that PR is what ships the issue's functionality.
...
### 2. Pre-creation: scan existing tracker + repo context

1. Search the tracker for duplicates or related open work.
2. Check existing cross-domain dependency notes that touch the areas in scope — ask whether to fold them in or keep separate.
...
- body: objective, scope in/out, success criteria, links to ADRs/docs, blockers called out.

**Task sub-issues** (execution units — one branch + one PR each):

- Linked as sub-issue of their phase issue.
...
- Body must include:
  ...
  - `requires`: blocked-by issue numbers or "none"
...
Set blocking relations between sub-issues when order matters.
...
- **One language only** in issue bodies (match the repo's language policy).
- **TDD mandatory**: every sub-issue lists `tests:` scenarios — red first.
- **Tracker only** for new planning — do not generate in-repo planning trees.
