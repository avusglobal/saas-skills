...
echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"Before completing this Write/Edit, manually validate: (a) TDD — if this file holds behavior (services/repositories/controllers or equivalent), make sure the matching test under tests/ mirroring src/ already exists and is failing; the tdd skill defines what a good test is, and CI runs the test suite on every push as the backstop. (b) Domain boundary — if there is an active task whose domain differs from the target module, record a cross-domain blocker issue in Linear instead of editing outside the domain."}}'
